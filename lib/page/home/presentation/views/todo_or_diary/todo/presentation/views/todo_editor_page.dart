...
class _TodoEditorPage extends State<TodoEditorPage> {
                      ...
                      child: MongleeBtn(
                        clickFuntion: () {
                          if (todoController.editCompleteCondition()) {
                            ...
                            MognleeToast.show(context: context, msg: 'hey');
                          }
                        },
  ...
  int _selectedRepeatIdx = -1;

  Widget _repeatWidget() {
    const List<TodoRepeat> repeatList = TodoRepeat.values;
    return MongleeExpanel(
        title: _selectedRepeatIdx == -1
            ? '반복 안함'
            : repeatList.elementAt(_selectedRepeatIdx).contents,
        items: List.generate(
            repeatList.length,
            (index) => MongleeChip(
                  text: repeatList[index].contents,
                  selected: index == _selectedRepeatIdx,
                  selectFunction: () {
                    if (mounted) {
                      setState(() {
                        _selectedRepeatIdx = index;
                      });
                    }
                  },
                )));
  ...
  int _selectedNotiIdx = -1;

  Widget _notiWidget() {
    const List<TodoNotiTime> notiList = TodoNotiTime.values;
    return MongleeExpanel(
        title: _selectedNotiIdx == -1
            ? '알림 없음'
            : notiList.elementAt(_selectedNotiIdx).contents,
        items: List.generate(
            notiList.length,
            (index) => MongleeChip(
                  text: notiList[index].contents,
                  selected: _selectedNotiIdx == index,
                  selectFunction: () {
                    if (mounted) {
                      setState(() {
                        _selectedNotiIdx = index;
                      });
                    }
                  },
                )));
